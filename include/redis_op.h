...
#define REDIS_COMMAND_SIZE  300        //redis 最大指令长度
#define FIELD_ID_SIZE       100        //redis hash表field字段最大长度
#define VALUES_ID_SIZE      1024       //value 字段最大长度

typedef char (*RCOMMANDS)[REDIS_COMMAND_SIZE];  //存放批量命令字符数据类型

typedef char (*RFIELDS)[FIELD_ID_SIZE];         //存放批量hash表key字符串类型
typedef char (*RVALUES)[VALUES_ID_SIZE];        //存放批量value字符串类型

/// @brief tcp 模式建立连接
/// @param ip 
/// @param port_str 
/// @return 
redisContext* rop_connectdb_nopwd(char* ip,char* port_str);
//携带密码的tcp 建立连接
